...
from .config import MODE4_DPI, MODE_DIRECTORIES, CONDENSER_NA, N_CONE_RAYS
...
from .utils import format_energy, format_angle, ensure_directory
...
def subdivide_mesh(X, Y, Z, subdivisions=1):
    ...
    for _ in range(subdivisions):
        N, M   = X.shape
        N2, M2 = 2 * N - 1, 2 * M - 1
        Xn = np.zeros((N2, M2)); Yn = np.zeros((N2, M2)); Zn = np.zeros((N2, M2))
        Xn[::2,  ::2]  = X;  Yn[::2,  ::2]  = Y;  Zn[::2,  ::2]  = Z
        Xn[::2,  1::2] = (X[:, :-1] + X[:, 1:]) / 2
        Yn[::2,  1::2] = (Y[:, :-1] + Y[:, 1:]) / 2
        Zn[::2,  1::2] = (Z[:, :-1] + Z[:, 1:]) / 2
        Xn[1::2, ::2]  = (X[:-1, :] + X[1:, :]) / 2
...
def export_to_obj(X, Y, Z, filename, B_faces=None):
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(N):
            for j in range(M):
                if i < N-1 and j < M-1:
                    ...
                    n  = np.cross(v1, v2); m = np.linalg.norm(n)
        ...
        for i in range(N-1):
            for j in range(M-1):
                v1=i*M+j+1; v2=i*M+j+2; v3=(i+1)*M+j+2; v4=(i+1)*M+j+1
...
def export_to_stl(X, Y, Z, filename):
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(N-1):
            for j in range(M-1):
                v1=np.array([X[i,j],Y[i,j],Z[i,j]]); v2=np.array([X[i,j+1],Y[i,j+1],Z[i,j+1]])
                v3=np.array([X[i+1,j+1],Y[i+1,j+1],Z[i+1,j+1]]); v4=np.array([X[i+1,j],Y[i+1,j],Z[i+1,j]])
                for va,vb,vc in [(v1,v2,v3),(v1,v3,v4)]:
                    n=np.cross(vb-va,vc-va); mg=np.linalg.norm(n); n=n/mg if mg>0 else n
...
def run_mode4_enhanced(range_interpolator, F_interp, vb, time_etching, outdir=None):
    ...
    try:
        ...
        if energy <= 0: raise ValueError("Energy must be positive")
    except ValueError as e:
        print(f"  Invalid energy: {e}"); return

    try:
        angle_deg = float(input("Enter incident angle (0-90 deg): ").strip())
        if not 0 <= angle_deg <= 90: raise ValueError("Angle must be 0-90")
    except ValueError as e:
        print(f"  Invalid angle: {e}"); return
    ...
    res = calculate_track_parameters(
        energy=energy, angle_deg=angle_deg,
        vb=vb, time_etching=time_etching,
        range_interpolator=range_interpolator,
        F_interp=F_interp, debug=False)

    if res.get('indicator', -1) != 1:
        print(f"\n  Track did not develop: {res.get('status')}"); return

    X_surf  = res['X_surf'];  Y_surf  = res['Y_surf']
    Z_surf  = res['Z_surf'];  B_faces = res['B_faces']
    if X_surf is None:
        print("  No mesh data available"); return
    ...
    if use_sub:
        ...
        Nq = (N_rows-1)*(N_cols-1); n_valid = len(brt)
        brt_q = 0.5*(brt[:Nq]+brt[Nq:2*Nq]) if n_valid >= 2*Nq else (brt[:Nq] if n_valid >= Nq else np.zeros(Nq))
    ...
    if export_3d and outdir:
        ...
        export_to_obj(X_surf, Y_surf, Z_surf, os.path.join(outdir, f"{base}.obj"), B_faces)
    ...
    z_min = Z_surf.min(); pad = (x_max-x_min)*0.2
    lim_x = (x_min-pad, x_max+pad); lim_y = (y_min-pad, y_max+pad)
    lim_z = (z_min*1.1, 0)
    ...
    if B_faces is not None:
        ax3d.plot_surface(X_surf,Y_surf,Z_surf, facecolors=cm.gray(B_faces),
                          linewidth=0, antialiased=True, shade=True, rstride=1, cstride=1)
    tx=lim_x[1]-lim_x[0]; ty=lim_y[1]-lim_y[0]; tz=max(abs(lim_z[0]),0.1)
    ax3d.set_box_aspect((tx,ty,tz))
    ax3d.set_xlim(lim_x); ax3d.set_ylim(lim_y); ax3d.set_zlim(lim_z)
    ...
    ax3d.view_init(elev=20, azim=-55); ax3d.grid(True, alpha=0.3)

    ax_xy = fig.add_subplot(gs[0,1])
    ax_xy.set_facecolor('#DCDCDC')
    if B_faces is not None:
        im = ax_xy.pcolormesh(X_surf,Y_surf,B_faces, cmap='gray',shading='flat',rasterized=True,vmin=0,vmax=1)
        plt.colorbar(im, ax=ax_xy, fraction=0.046, pad=0.04).set_label('Brightness',rotation=270,labelpad=15,fontsize=9)
    ax_xy.plot(X_surf[0,:], Y_surf[0,:], 'r-', lw=2, label='Opening', alpha=0.7)
    ax_xy.set_xlim(lim_x); ax_xy.set_ylim(lim_y); ax_xy.set_aspect('equal')
    ax_xy.set_title("XY Microscope View", fontweight='bold')
    ax_xy.set_xlabel("X (µm)"); ax_xy.set_ylabel("Y (µm)")
    ax_xy.legend(loc='upper right', fontsize=8); ax_xy.grid(True, alpha=0.3)

    ax_yz = fig.add_subplot(gs[1,0])
    ax_yz.plot(Y_surf, Z_surf, color='black', alpha=0.05)
    ax_yz.set_xlim(lim_y); ax_yz.set_ylim(lim_z); ax_yz.set_aspect('equal')
    ax_yz.set_title("YZ Front Profile", fontweight='bold')
    ax_yz.set_xlabel("Y (µm)"); ax_yz.set_ylabel("Z (µm)")
    ax_yz.grid(True, alpha=0.3)

    ax_xz = fig.add_subplot(gs[1,1])
    ax_xz.plot(X_surf, Z_surf, color='black', alpha=0.05)
    ax_xz.set_xlim(lim_x); ax_xz.set_ylim(lim_z); ax_xz.set_aspect('equal')
    ax_xz.set_title("XZ Side Profile", fontweight='bold')
    ax_xz.set_xlabel("X (µm)"); ax_xz.set_ylabel("Z (µm)")
    ax_xz.grid(True, alpha=0.3)

    fig.suptitle(f"Proton Track  {energy:.2f} MeV  {angle_deg:.1f}°  "
    ...
    if outdir:
        ...
        filename = f"track_4panel_{format_energy(energy)}MeV_{format_angle(angle_deg)}deg.png"
