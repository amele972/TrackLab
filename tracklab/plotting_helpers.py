...
def plot_upper_surface_colored(vertices, normals, brightness, blur_sigma=2.0):
    ...
    # Create Collection
    coll = PolyCollection(verts_sorted, array=bright_sorted, cmap='gray', edgecolors='face')
    ...
    cax = ax_final.imshow(img_blurred, extent=[-max_dim, max_dim, -max_dim, max_dim], 
                          cmap='gray', origin='upper', vmin=0, vmax=255)
...
def plot_trisurf(df_surface, xcol, ycol, zcol, title, outdir=None, save_svg=True, dpi=600):
    """
    ...
    for invert in [False, True]:
        ...
        surf = ax.plot_trisurf(x, y, z, cmap='plasma', alpha=0.8, linewidth=0.2, edgecolor='none')
