...
class LUTEngine:
    ...
    def energy_range(self):
        return (
            float(self._df["energy_MeV"].min()),
            float(self._df["energy_MeV"].max()),
        )

    @property
    def angle_range(self):
        return (float(self._df["angle_deg"].min()), float(self._df["angle_deg"].max()))
    ...
    def query(self, energy, angle) -> dict:
        ...
        if shape == () or (len(shape) == 1 and shape[0] == 1):
            return {
                k: (float(v[0]) if v.dtype != bool else bool(v[0]))
    ...
    def process_fluka_file(
        ...
        for idx, line in enumerate(lines):
            ...
            try:
                ...
                etching_time = (
                    time_etching_ref * abs(z_cm) / abs(z_ref)
                    if z_ref != 0
                    else time_etching_ref
                )
                if etching_time <= 0:
                    skipped += 1
                    continue
                ...
                records.append(
                    (energy_mev, angle_deg, x_cm, y_cm, z_cm, cosz, etching_time)
                )
    ...
    def inverse_lookup(
        ...
        if energy_hints is not None and angle_hints is not None:
            hints = np.column_stack(
                [np.atleast_1d(energy_hints), np.atleast_1d(angle_hints)]
            )
        ...
        for c0 in candidates:
            try:
                ...
                duplicate = any(
                    abs(e_sol - s[0]) < 0.1 and abs(a_sol - s[1]) < 0.5 for s in seen
                )
                if not duplicate:
