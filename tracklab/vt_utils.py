...
def vt_function(y):
    ...
    if PROTON_VT_MODEL == 2:
        ...
        term2 = a1 / ((y_safe + a2)**b1) * np.log(y_safe + a3) * (1.0 - np.exp(-y_safe / a4))
        term3 = y_safe / a5
        v = 1.0 + term2 + term3
...
def alpha_vt_function(y, model_index=None):
    ...
    """
    from .config import ALPHA_VT_MODEL, ALPHA_MODELS_INFO
    ...
    if model_index == 1:
        ...
        v = 1.0 + (p["a1"] * np.exp(-p["a2"] * y) + p["a3"] * np.exp(-p["a4"] * y)) * (1.0 - np.exp(-p["a5"] * y))
        
    elif model_index == 2:
        # 2: Brun et al. (1999)
        v = 1.0 + np.exp(-p["a1"]*y + p["a4"]) - np.exp(-p["a2"]*y + p["a3"]) + np.exp(p["a3"]) - np.exp(p["a4"])
        ...
        # 4: Al-Jubbori (2020)
        v = 1.0 + np.exp(-p["a1"] * y_safe + p["a2"] - p["a3"] / y_safe + p["a4"] / (y_safe**p["a5"]))
        
    elif model_index == 5:
        # 5: Hermsdorf (2009)
        v = 1.0 + (p["a1"] / (p["a2"] + y)**p["b1"]) * (1.0 - np.exp(-y/p["a4"])) * (np.log(y + p["a3"]) + y/p["a5"])
...
def build_vrint_interpolator(vt_model=None, ion=None, energy=None,
                             vb=None, R_max: float = 6600.0, N: int = 10000):
    """
    ...
    if energy is None:
        raise ValueError(
            "energy is required when using multi-ion model (vt_model)")
