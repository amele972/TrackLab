...
from .vt_utils import vt_function, fast_dint3
...
def calculate_track_parameters(energy=None, angle_deg=None, vb=None, time_etching=None,
                                range_interpolator=None, F_interp=None,
                                ion=None, vt_model=None,
                                debug: bool = False,
                                plot:  bool = False):
    """
    ...
    if theta_rad < thetag or energy < 0.1:
        return _result_template(energy, angle_deg, range_val, removed,
                                "Angle < Critical")
    ...
    if abs(theta_rad - pi / 2.0) < 1e-6:
        ...
        for i in range(start, len(tx)):
            total_x_list.append(tx[i]); total_y_list.append(y_up[i])
        for k in range(1, len(tx) - start):
            idx = len(tx) - k
            if idx >= 0:
                total_x_list.append(tx[idx]); total_y_list.append(y_low[idx])
    else:
        # Oblique incidence
    ...
    if len(x_prof) > 5:
        ...
        bp, ts, ps, _v, _n, brt = optics_func(
            X_surf, Y_surf, Z_surf, debug_mode=debug,
            condenser_na=CONDENSER_NA, n_cone_rays=N_CONE_RAYS
        )
        ...
        if len(brt) >= 2 * Nq:
            B_faces = 0.5 * (brt[:Nq] + brt[Nq:2 * Nq]).reshape(N_rows - 1, N_cols - 1)
        elif len(brt) >= Nq:
    ...
    return {
        'energy': energy, 'angle_deg': angle_deg, 'theta_rad': theta_rad, 'ion': ion,
        'depth_um': depth_um, 'major_axis_um': major_axis_um, 'minor_axis_um': minor_axis_um,
        'black_part': black_part, 'total_surface': total_surface, 
        'projected_surface': projected_surface, 'total_length_um': total_length_um,
        'mean_brightness': mean_brightness,
        'X_surf': X_surf, 'Y_surf': Y_surf, 'Z_surf': Z_surf, 'B_faces': B_faces,
        'indicator': 1, 'status': 'Developed',
        'removed_um': original_removed, 'range_um': original_range,
        'etched_dist_um': etched_dist, 'induct_range_um': total_induct_range,
        'x_prof': x_prof, 'r_prof': r_prof
    }
