...
def run_mode2_single_track(ion='protons', energy=1.5, angle=75.0,
                           vb=None, time_etching=None, show_plot=True):
    """Mode 2: Single track calculation and visualization."""
    import matplotlib.pyplot as plt
    from .config import get_vb_for_ion, TIME_ETCHING
    ...
    result = calculate_track_parameters(
        ...
        F_interp=F_interp, ion=ion, vt_model=vt_model,
        debug=True,
    )
    ...
    if show_plot and result['X_surf'] is not None:
        ...
        fig.suptitle(f"{ion} Track: {energy:.2f} MeV, {angle:.1f}°",
                     fontweight='bold')
        plt.tight_layout()
        plt.show()
...
def run_mode3_reference_dataset(ion='protons', e_min=0.1, e_max=10.0,
                                n_energies=20, a_min=0.0, a_max=90.0,
                                n_angles=15, vb=None, time_etching=None,
                                output_csv=None):
...
def run_mode4_fluka(ion='protons', input_file=None, vb=None, time_etching=None,
                    output_csv=None, skip_header=1):
    ...
    for idx, line in enumerate(lines):
        try:
            ...
            res = calculate_track_parameters(
                energy=energy_mev, angle_deg=angle_deg, vb=vb,
                time_etching=time_etching,
                range_interpolator=range_interp,
                F_interp=F_interp, ion=ion, vt_model=vt_model,
            )
...
def run_mode5_3d_enhanced(ion='protons', energy=1.5, angle=75.0,
                          vb=None, export_dir=None):
    ...
    if export_dir:
        try:
            ...
            from .mode4_enhanced import (
                subdivide_mesh, export_to_obj, export_to_stl,
                create_blender_script)
            X, Y, Z = subdivide_mesh(res['X_surf'], res['Y_surf'],
                                      res['Z_surf'], subdivisions=1)
            base = os.path.join(export_dir,
                f"track_{ion}_{energy:.1f}MeV_{angle:.0f}deg")
