...
class _LUTLoaderMixin:
    ...
    def _load_lut(self):
        ...
        try:
            ...
            self.lut_label.setStyleSheet("color: #a6e3a1; font-weight: bold;")
            self._on_lut_loaded()
...
class _Tab6c(QWidget, _LUTLoaderMixin):
    def __init__(self, param_panel=None):
        ...
        self._make_lut_bar(lay)
        lay.addWidget(QLabel(""))
        lay.addWidget(_section_label("Measured axes → estimate (E, θ)"))

        q_row = QHBoxLayout()
        for lbl, attr, val in [
            ("Major axis (µm):", "maj_spin", 10.0),
