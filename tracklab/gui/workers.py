...
class TrackWorker(QThread):
    ...
    def run(self):
        try:
            ...
            from tracklab.load_srim_data import load_srim_data, interpolate_range
...
class BatchWorker(QThread):
    ...
    def run(self):
        try:
            ...
            energies = np.linspace(self.params['e_min'], self.params['e_max'],
                                   int(self.params['e_steps']))
            angles   = np.linspace(self.params['a_min'], self.params['a_max'],
                                   int(self.params['a_steps']))
                ...
                for angle in angles:
                    ...
                    if count % max(1, total // 10) == 0:
                        self.status_update.emit(
                            f"Computed {count}/{total} tracks…")

            if self.running:
                developed = sum(1 for r in results
...
class FlukaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            col = self.params.get('col_map', {
                'energy': 1, 'x': 2, 'y': 3, 'z': 4, 'cosz': 7})
            ...
            for idx, line in enumerate(lines):
                ...
                try:
                    ...
                    F_interp = build_vrint_interpolator(
                        ...
                        energy=energy_mev, vb=vb)

                    res = calculate_track_parameters(
                        energy=energy_mev, angle_deg=angle_deg,
                        vb=vb, time_etching=etching_time,
                        range_interpolator=range_interp,
                        F_interp=F_interp,
            ...
            if self.running:
                self.result_ready.emit({
                    ...
                    'developed': developed, 'skipped': skipped,
                    'total': total,
                })
                self.progress.emit(100)
