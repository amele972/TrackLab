...
def _plot_4panel(fig, ion, energy, angle, X, Y, Z, B, colors=None):
    ...
    if B is not None:
        ax1.plot_surface(
            X, Y, Z, facecolors=cm.gray(B), linewidth=0, antialiased=False, shade=False
        )
    ax1.set_xlim(lim_x)
    ax1.set_ylim(lim_y)
    ax1.set_zlim(lim_z)
    ...
    for ax, coord, lim, xlbl in [
        ...
        b_row = c1[-1, :]
        b_z = c2[-1, :]
        bc = np.append(b_row, b_row[0])
        bz = np.append(b_z, b_z[0])
...
class Enhanced3DTab(QWidget):
    ...
    def _on_result(self, res):
        ...
        _plot_4panel(
            self.fig,
            self.param_panel.ion,
            self.param_panel.energy,
            self.param_panel.angle,
            X,
            Y,
            Z,
            B,
    ...
    def _advanced(self, X, Y, Z, B):
        try:
            ...
            from tracklab.track_optics_p_optimized import track_optics_p_optimized

            if self.subdiv_check.isChecked():
                X, Y, Z = subdivide_mesh(X, Y, Z, subdivisions=1)
                bp, ts, ps, _v, _n, brt = track_optics_p_optimized(
                    X, Y, Z, condenser_na=CONDENSER_NA, n_cone_rays=N_CONE_RAYS
                )
                Nr, Nc = X.shape
                Nq = (Nr - 1) * (Nc - 1)
                nv = len(brt)
            ...
            if self.ao_check.isChecked() and B is not None:
                ...
                ao_f = (ao[:-1, :-1] + ao[1:, :-1] + ao[:-1, 1:] + ao[1:, 1:]) / 4
                B = B * (0.3 + 0.7 * ao_f)
    ...
    def _export_3d(self, X, Y, Z, B):
        outdir = QFileDialog.getExistingDirectory(self, "Select export directory")
        if not outdir:
            return
        try:
            from tracklab.mode_use import (
            ...
            base = os.path.join(outdir, f"track_{ion}_{e:.1f}MeV_{a:.0f}deg")
            ...
            QMessageBox.information(
                self, "Export Complete", f"Files saved to:\n{outdir}"
            )
        except Exception as e:
    ...
    def _export_png(self):
        ...
        if path:
            try:
                c = self._colors()
                self.fig.savefig(path, dpi=150, bbox_inches="tight", facecolor=c["bg"])
