...
class SingleTrackTab(QWidget):
    ...
    def _plot_4panel(self, res):
        X, Y, Z, B = (res['X_surf'], res['Y_surf'],
                       res['Z_surf'], res['B_faces'])
        if X is None:
            return
        ...
        ax2.annotate('', xy=(x_min_op, 0), xytext=(x_max_op, 0),
        ...
        ax2.annotate('', xy=(x_at_max_y, -y_max_op), xytext=(x_at_max_y, y_max_op),
        ...
        ax2.legend(custom_lines, ['Opening', 'Major Axis', 'Minor Axis'],
        ...
        ax3.annotate('', xy=(y_pos_arrow, -depth), xytext=(y_pos_arrow, 0),
        ...
        ax3.set_xlim(lim_y); ax3.set_ylim(lim_z); ax3.set_aspect('equal')
        ax3.set_title("YZ Profile", color=c['text'], fontweight='bold')
        ax3.set_xlabel("Y (µm)"); ax3.set_ylabel("Z (µm)")
        ...
        ax4.annotate('', xy=(x_pos_arrow, -depth), xytext=(x_pos_arrow, 0),
                     arrowprops=dict(arrowstyle='<->', color=c_dep, lw=1.5))

        # XZ Legend (Bottom)
        ax4.legend(prof_lines, ['Depth'], fontsize=8, loc='lower right', framealpha=0.8)

        ax4.set_xlim(lim_x); ax4.set_ylim(lim_z); ax4.set_aspect('equal')
