...
class VTMultiIonModel:
    ...
    def __init__(self, xlsx_path=None):
        ...
        self.param_overrides = {
            'Li': {
                2.87:  {'A': 1.9197, 'y0': 6.1250,  'alpha': 1.4343, 'beta': 18.6993, 'r2': 0.999},
                4.82:  {'A': 3.1416, 'y0': 3.0855,  'alpha': 1.6366, 'beta': 1.3550,  'r2': 0.999},
                6.75:  {'A': 2.7982, 'y0': 3.9384,  'alpha': 1.8076, 'beta': 1.8535,  'r2': 0.999},
                7.99:  {'A': 1.3269, 'y0': 5.5365,  'alpha': 1.7065, 'beta': 1.8445,  'r2': 0.999},
                9.02:  {'A': 4.5552, 'y0': 3.3722,  'alpha': 1.5230, 'beta': 1.3259,  'r2': 0.999},
                10.96: {'A': 7.9756, 'y0': 15.0555, 'alpha': 2.0e-9, 'beta': 1.6365,  'r2': 0.999},
                11.99: {'A': 3.7111, 'y0': 2.9485,  'alpha': 1.6837, 'beta': 0.9994,  'r2': 0.999},
            },
            'C': {
                14.80: {'A': 32.8938, 'y0': 5.3102, 'alpha': 0.4235, 'beta': 2.3801, 'r2': 0.999},
                19.20: {'A': 15.3749, 'y0': 5.4891, 'alpha': 1.1025, 'beta': 2.2640, 'r2': 0.999},
                22.50: {'A': 18.0433, 'y0': 9.1102, 'alpha': 0.6736, 'beta': 2.9860, 'r2': 0.999},
            },
            'O': {
                17.26: {'A': 14.2817, 'y0': 4.6403, 'alpha': 1.2285, 'beta': 2.0371, 'r2': 0.999},
                22.23: {'A': 16.1185, 'y0': 5.7074, 'alpha': 1.1823, 'beta': 2.6359, 'r2': 0.999},
                26.41: {'A': 15.2919, 'y0': 5.4477, 'alpha': 1.4142, 'beta': 2.3442, 'r2': 0.999},
            }
        }
    ...
    def _load_and_fit(self, xlsx_path):
        ...
        if not required.issubset(df.columns):
            print(f"[VTMultiIonModel] Warning: missing columns "
        ...
        for ion in self.ions_in_data:
            ...
            for energy in sorted(ion_df['Energy'].unique()):
                ...
                if result:
                    ...
                    print(f"  {ion:8s} {energy:6.2f} MeV: "
        ...
        for ion, energies in self.param_overrides.items():
            if ion in self.model_data:
                for energy, params in energies.items():
                    ...
                    print(f"[VTMultiIonModel] Applied manual override for {ion} @ {energy} MeV")
    ...
    def _V_fit(self, y, ion, energy):
        ...
        if energy <= anchors[0]:
            params = {k: v for k, v in self.model_data[ion][anchors[0]].items() if k in ['A', 'y0', 'alpha', 'beta']}
            return vt_bpl(y, **params)
        if energy >= anchors[-1]:
            params = {k: v for k, v in self.model_data[ion][anchors[-1]].items() if k in ['A', 'y0', 'alpha', 'beta']}
            return vt_bpl(y, **params)
        ...
        p1 = {k: v for k, v in self.model_data[ion][e1].items() if k in ['A', 'y0', 'alpha', 'beta']}
        p2 = {k: v for k, v in self.model_data[ion][e2].items() if k in ['A', 'y0', 'alpha', 'beta']}
    ...
    def get_parameters(self, ion='protons', energy=1.0):
        ...
        if ion in self.interpolators:
            try:
                return {p: float(self.interpolators[ion][p](energy))
        ...
        return {k: v for k, v in self.model_data[ion][nearest].items()
...
if __name__ == '__main__':
    ...
    for ion, energy in [('protons', 1.0), ('C', 14.8), ('O', 22.0), ('Li', 6.75)]:
        ...
        for y_test in [1.0, 2.0, 5.0]:
            ...
            print(f"  V({y_test:.0f} µm)  VB=1.73 -> {v173:.3f}   "
                  f"VB=4.7 -> {v47:.3f}")
