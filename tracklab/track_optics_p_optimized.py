...
def build_cone_rays(condenser_na, n_rays):
    ...
    for k in range(n_rays - 1):
        ...
        rays.append([np.sin(alpha) * np.cos(phi),
...
def compute_quad_normals(X, Y, Z):
    ...
    d1x = T3x - T1x;  d1y = T3y - T1y;  d1z = T3z - T1z
    # Diagonal T2 -> T4
    d2x = T4x - T2x;  d2y = T4y - T2y;  d2z = T4z - T2z
    ...
    v1x = T2x - T1x;  v1y = T2y - T1y;  v1z = T2z - T1z
    v2x = T4x - T1x;  v2y = T4y - T1y;  v2z = T4z - T1z
    cx  = v1y * v2z - v1z * v2y
    cy  = v1z * v2x - v1x * v2z
    cz  = v1x * v2y - v1y * v2x
...
def track_optics_p_optimized(X_lab, Y_lab, Z_lab, debug_mode=False,
    ...
    def get_triangles(arr):
        p0 = arr[:-1, :-1]; p1 = arr[1:, :-1]
        p2 = arr[1:,  1:];  p3 = arr[:-1, 1:]
        return np.concatenate([np.stack([p0, p1, p2], axis=-1),
                                np.stack([p0, p2, p3], axis=-1)], axis=0)

    verts_all = np.stack([get_triangles(X_lab).reshape(-1, 3),
                          get_triangles(Y_lab).reshape(-1, 3),
                          get_triangles(Z_lab).reshape(-1, 3)], axis=-1)
    ...
    black_part    = (float(np.sum(areas_tri[dark_mask])) / total_surface
                     if total_surface > 0 else 0.0)
    ...
    if debug_mode:
        ...
        print(f"[track_optics] faces={len(brightness)}  cone_rays={n_rays}"
              f"  condenser_NA={condenser_na:.2f}")
        print(f"[track_optics] crit_angle={crit_deg:.2f} deg"
        ...
        if len(bright):
            print(f"[track_optics] bright faces: mean={bright.mean():.3f}"
