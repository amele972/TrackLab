...
def test_proton_track():
    ...
    for tc in test_cases:
        F_interp = build_vrint_interpolator(
            vt_model=vt_model, ion="protons", energy=tc["energy"], vb=tc["vb"]
        )
        ...
        print(
            f"{tc['energy']:8.2f} {tc['angle']:6.1f} {tc['vb']:6.2f} "
...
def test_multiion_tracks():
    ...
    for tc in test_cases:
        ...
        F_interp = build_vrint_interpolator(
            vt_model=vt_model, ion=ion, energy=tc["energy"], vb=vb
        )
        ...
        print(
            f"{ion:>8} {tc['energy']:8.2f} {tc['angle']:6.1f} "
