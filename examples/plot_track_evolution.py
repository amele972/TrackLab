...
def main():
    ...
    for row, disp_angle in enumerate(display_angles):
        ...
        for energy, color in zip(energies, colors):
            ...
            for t in times:
                res = calculate_track_parameters(
                    energy=energy, angle_deg=tracklab_angle, vb=vb, time_etching=t,
            ...
            axes[row, 0].plot(times, major_axes, label=f'{energy} MeV', color=color, lw=3)
            axes[row, 1].plot(times, minor_axes, label=f'{energy} MeV', color=color, lw=3)
        ...
        axes[row, 0].set_ylabel(f'{disp_angle}° Incident Angle\n\nMajor Axis (µm)', fontsize=16, fontweight='bold')
        axes[row, 1].set_ylabel(f'Minor Axis (µm)', fontsize=16)
        axes[row, 2].set_ylabel(f'Depth (µm)', fontsize=16)
        
        for col in range(3):
            axes[row, col].grid(True)
            # Add vertical dashed lines at specific etching times: 2h 50m, 4h 30m, 7h
            for vline in [2 + 50/60, 4.5, 7.0]:
                axes[row, col].axvline(x=vline, color='k', linestyle='--', alpha=0.7, lw=2)
            axes[row, col].tick_params(axis='both', which='major', labelsize=14)
            if row == 0:
                if col == 0: axes[row, col].set_title('Major Axis', fontsize=20, fontweight='bold', pad=15)
                if col == 1: axes[row, col].set_title('Minor Axis', fontsize=20, fontweight='bold', pad=15)
                if col == 2: axes[row, col].set_title('Depth', fontsize=20, fontweight='bold', pad=15)
            if row == 2: # Last row for x labels
                axes[row, col].set_xlabel('Etching Time (h)', fontsize=18, fontweight='bold')
            axes[row, col].legend(fontsize=14)

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'track_evolution.png')
